...
/**
 * @name MaxLen
 * Check if the length of the given value is less than the max value
 * 
 * @package Catapult.Libraries.Validate.Validators
 * @filesource MaxLen.php
 * 
 ...
 */
class MaxLen implements iValidator, iValidatorArgs {
	/**
	 * @var Int $_iMaxLen
	 * Indicate the maximum length to check
	 */
	private $_iMaxLen;
	...
	public function validate ($mValue) {
		return (strlen ($mValue) <= $this->_iMaxLen) ? true : false;
	}
}
?>